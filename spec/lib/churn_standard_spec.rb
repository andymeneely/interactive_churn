...
COMMAND_NAME = 'ichurn'
  ...
  context "within a git repository" do
    ...
    it "returns a string with number of commits envolved, number of lines inserted and deleted, and the total churn" do
      expect(ChurnStandard.get_output).to eq("Commits:       10\nTotal Churn:   68\nLines added:   48\nLines deleted: 20\n")
    end

    it "returns churns metrics in json format" do
      expect(ChurnStandard.get_output({format: "--json"})).to eq( "{\"Commits\":10,\"Total Churn\":68,\"Lines added\":48,\"Lines deleted\":20}")
    end
